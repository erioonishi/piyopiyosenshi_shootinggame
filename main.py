...
pygame.display.set_caption("シューティングゲーム")  #タイトルバー
...
ENEMY_SPAWN_EVENT = pygame.USEREVENT + 1  #Pygame独自のカスタムイベントを作成、敵が出現するタイミング管理するイベントで
pygame.time.set_timer(ENEMY_SPAWN_EVENT, 4000)  #4秒ごと(4000ミリ秒)にENEMY_SPAWN_EVENTを自動で発生させ、定期的に敵が出現
...
while True: #無限ループでゲームループを開始(ここがゲームの中心部分)
    ...
    for event in pygame.event.get():  #すべてのイベント(キー入力/マウス/タイマーイベントなど)を順番に取得して処理
        ...
        if not game.game_over:  #ゲームオーバー時ではない処理
            if event.type == ENEMY_SPAWN_EVENT:  #4秒ごとに発生するENEMY_SPAWN_EVENTで敵を1体出現
                game.spawn_enemy()
            else:
                game.handle_input(event)  #その他のイベント(キー入力など)はGameクラスのhandle_input()に渡して処理
        else:  #ゲームオーバー時でも入力処理
            game.handle_input(event)  #ゲームオーバー時も再スタートなどのキー入力は必要なので、handle_input()に渡す
